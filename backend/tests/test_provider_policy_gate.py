...
`RAW_MEDIA_STORE_ENABLED`/`GOOGLE_PLACE_SEARCH_ENABLED`가 꺼졌을 때 해당
...
class _GatesOffSettings:
    ...
    RAW_MEDIA_STORE_ENABLED = False
    GOOGLE_PLACE_SEARCH_ENABLED = False


@pytest.mark.asyncio
async def test_store_raw_media_skips_when_store_disabled(monkeypatch, caplog):
    ...
    assert any(
        "RAW_MEDIA_STORE_ENABLED=false" in record.message
...
def test_kill_switch_defaults_keep_current_behavior():
    ...
    assert settings.RAW_MEDIA_STORE_ENABLED is True
