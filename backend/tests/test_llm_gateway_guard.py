...
`backend/ktc` 소스를 스캔해 allowlist(`gemini_client.py`, `deepseek_client.py`,
`llm_client.py`, `gemini_rate_limiter.py`) 밖에서 다음을 검출하면 실패한다:

(a) `google.genai`/`genai.` 직접 사용 (Google GenAI SDK 직접 호출)
(b) `post_generate_content` 직접 호출 (Gemini HTTP 헬퍼 우회)
...
KTC_ROOT = Path(__file__).resolve().parents[1] / "ktc"
...
def test_no_direct_sdk_usage_outside_gateway():
    ...
    violations = find_violations(KTC_ROOT)
    assert violations == [], (
