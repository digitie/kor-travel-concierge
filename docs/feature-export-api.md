...
| `sido_code` | **`sigungu_code` 앞 2자리로 유도**(`sigungu_code[:2]`) | `sigungu_code`가 `null`이면 `null` |

`sido_code` 전용 컬럼은 두지 않는다. 행정표준 시군구 코드의 앞 2자리가 시도 코드라는 규칙을
이용해 `sigungu_code`에서 유도한다(경량 옵션). 시군구 코드가 없으면 유도 결과도 `null`이다.
...
**기본값은 `true`**로, 확정 좌표(`travel_places.is_geocoded`)가 없는 장소를 제외해 미검증·미확정
좌표가 GPX/KML로 유출되는 것을 막는다. 미검증 좌표까지 내보내려면 `geocoded_only=false`로
opt-out 한다. 이 파라미터는 `/features/*` 공급 계약에는 영향을 주지 않는다.
