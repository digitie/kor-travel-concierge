...
- provider 결과를 `result_kind`(poi | address | coordinate)로 구분한다. Kakao 주소검색·VWorld
  ...
  적용한다.
- **이름 게이트(D2·C8)**: any-pair `_names_compatible`를 비교 목적별 **pairwise** `_names_match`
  ...
  기존 확정 장소명만 대조한다(불일치 시 `nearby_place_name_mismatch`). 주소·좌표 결과는 POI
  이름 게이트를 skip하고 `name_unverified`로 남긴다(자동확정은 다른 신호로).
...
- **ADR-16 경계 재정의**: 다건(ambiguous) 결과에서 이름·행정구역 게이트를 **모두 통과하는
  후보가 정확히 1개**면 `matched(0.7)`로 자동확정한다. ADR-16의 "매칭 실패는 자동 확정하지
  않는다"는 유지하되, **게이트가 정밀도를 지키는 전제에서만** ambiguous 자동확정을 허용하도록
  경계를 좁힌다. 무게이트 자동확정은 여전히 금지다.
...
- (긍정) "검색 이름과 다른 장소가 1.0으로 자동확정"이 불가능해지고, 지역 불일치·이름 불일치·
  국내 미확인이 사유 코드로 검수 큐에 표시된다. FP의 downstream 전파를 원천 차단한다.
- (긍정) 다건이어도 게이트 단일 통과분은 자동확정돼 검수 유입이 준다.
- (부정) 시도 수준 별칭 asset은 수기 관리 대상이며 시군구까지는 검증하지 않는다(동음 지명
  오검출을 피해 정밀도를 택함). "광주"(광역시)↔"경기 광주시"처럼 축약이 겹치는 소수 사례는
  안전하게 검수 큐로 보낸다. is_domestic None은 별도 enum 대신 "해외 후보"로 표기돼 의미가
  다소 넓다.
