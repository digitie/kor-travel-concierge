...
> 보강(2026-06-09, ADR-23): 본 계약 중 "Windows live 고정 포트(API `9041` / Web `9042`)" 매핑, `ensure-windows-ffmpeg.ps1` 기반 호스트 FFmpeg 부트스트랩, Windows live 재시작 스크립트 항목은 ADR-23으로 대체되었다. Compose host port 기본값은 표준 `8000`/`3000`으로 되돌렸고, FFmpeg은 컨테이너 `/usr/bin/ffmpeg`로 단일화했으며 PowerShell 스크립트는 제거했다. Compose 계약의 나머지(서비스 구성, host port override, `/health` 기반 시작 순서, MCP `streamable-http`)는 그대로 유효하다.
...
- 기본 실행은 단일 호스트 Docker Compose(ADR-18): `docker compose up --build`로 backend(8000), frontend(3000), rustfs, mcp를 띄운다. Compose host port 기본값을 Windows 라이브 전용 `9041`/`9042`에서 표준 `8000`/`3000`으로 되돌린다.
...
- ADR-6, T-027, T-030, T-041, T-055 등 Windows 전제 작업 산출물(라이브 포트, FFmpeg 자동 준비, PowerShell launcher)은 본 ADR로 보정·대체된다.
...
- **프론트엔드 연동**: frontend는 `/api/v1/...` 경로를 호출하고, 비-local 백엔드용으로 `NEXT_PUBLIC_API_KEY`가 있으면 `X-API-Key` 헤더를 붙인다(로컬/E2E는 비워 두어 무인증).
