...
## 2026-06-28: T-144 — 헤더 로그아웃·작업 상태·탭 배치 정리

- **헤더 정리**: `AppShell`의 로그아웃 버튼을 헤더 카드 action 영역에서 빼서 우측 최상단에 배치했다.
  작업 상태 링크는 제목 아래 큰 카드형 링크 대신 제목 옆의 작은 pill 형태로 바꿔 실행/대기 수와
