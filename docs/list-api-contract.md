...
T-177은 backend 기본 응답을 envelope로 바꾸되 화면 UX는 변경하지 않는다. 프런트 API 모듈은
`listRunsPage`, `listDestinationsPage`, `listUnmatchedCandidatesPage`, `listThemesPage`를 제공하고,
기존 배열 반환 함수는 임시로 `page.items`를 풀어 현재 화면을 보존한다. 후속 T-178·T-183·
T-190·T-192에서 각 화면이 metadata와 cursor를 직접 사용한다.
