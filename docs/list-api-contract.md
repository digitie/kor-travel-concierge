...
- 검수는 `channel_id`, `playlist_id`, `keyword`, `q`, `sort`, `is_domestic`, `status`,
  ...
  `needs_review|ignored`, `grounding`은 `verified_raw`, `unverified`, `missing`,
  `not_applicable`, `legacy_unknown`만 허용한다.
  `sort`는 `newest|oldest`만 허용하고 `is_domestic`은 `true|false` 또는 생략만 허용한다.
  `deleted_at IS NULL`은 모든 검수 목록의 고정 조건이다. `q`는 앞뒤 공백만 제거한 뒤
...
올라갔다. 이전 scope cursor는 filter 의미가 달라 400으로 명시 만료한다.
