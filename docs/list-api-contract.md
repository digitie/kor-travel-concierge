...
|---|---|---:|---|
| 검수 newest | `id DESC` | 2,000 | `/destinations/candidates/{candidate_id}/detail` |
| 검수 oldest | `id ASC` | 2,000 | 동일 |
...
- 검수는 `channel_id`, `playlist_id`, `keyword`, `q`, `sort`, `is_domestic`, `status`,
  `reason`, `source_kind`, `grounding`을 fingerprint에 포함한다. `status`는
  `needs_review|ignored|removed`, `grounding`은 `verified_raw`, `unverified`, `missing`,
  `not_applicable`, `legacy_unknown`만 허용한다.
  `sort`는 `newest|oldest`만 허용하고 `is_domestic`은 `true|false` 또는 생략만 허용한다.
  `needs_review|ignored`는 `deleted_at IS NULL`을 강제하고, `removed`는 live ignored와 모든
  soft-deleted 후보를 합친다. `q`는 앞뒤 공백만 제거한 뒤
  후보명·위치 단서에 PostgreSQL `ILIKE` literal 부분 문자열 검색을 적용하므로 `%`, `_`, `\\`도
  wildcard가 아니라 입력 문자 자체로 취급한다. 이 계약부터 검수 cursor scope는
  `unmatched-v5`이다.
...
검수 웹 화면은 API 기본 newest와 별개로 FIFO 처리를 위해 oldest를 기본으로 사용하며 300건씩
append한다. 첫 page의 `newest_id`로 60초마다 `limit=1&newer_than_id=...` probe를 보내
`newer_than`뿐 아니라 현재 filter의 정확한 `total` 변화도 감지한다. 변화가 있더라도 열려 있는
snapshot을 자동 교체하지 않고 사용자가 배너에서 새 snapshot 시작을 선택한다.

## 검수 목록 경량 payload와 대기 사유

T-182부터 검수 목록은 후보와 `youtube_videos`·`youtube_channels`를 항상 outer join하되,
다음 짧은 scalar만 추가한다.

- `video_title`, `channel_title`
- `confidence_score`: 유한한 `0..1` 값만 반환하며 NaN·무한대·범위 밖 값은 `null`
- `source_kind`, `grounding_status`, `created_at`
- `review_state`, `state_revision`, `deleted_at`, `deletion_reason`, `deleted_by`,
  `video_is_excluded`, `undo`, `last_client_operation_id`
- `queue_reason`

긴 `source_text`, `review_note`, `provider_evidence_json`은 상세 API에만 둔다. n150 PostgreSQL의
300건 fixture에서 목록 JSON은 기존 필드 환산 64,456 byte에서 122,656 byte로 58,200 byte
(항목당 약 194 byte) 증가했다. evidence에 100KB 문자열을 넣은 후보도 목록 응답 크기에 해당
문자열이 반영되지 않는 회귀 테스트로 경계를 고정한다.

`queue_reason`은 저장 컬럼이 아니라 evidence에서 파생하는 안정 enum이다. 아래 위쪽 값이 먼저
일치하면 그 값 하나만 반환한다.

1. `ungrounded` — T-165 raw grounding 도입 뒤에만 산출
2. `name_mismatch`
3. `region_mismatch`
4. `source_conflict`
5. `source_low_confidence`
6. `source_uncertain`
7. `ambiguous`
8. `no_result`
9. `vworld_unrefined_single`
10. `foreign`
11. `description_only`
12. `visual_only`
13. `provider_missing` — geocoding 객체는 있으나 decision reason이 없는 손상·누락 evidence
14. `extraction_only` — 위 사유가 없고 아직 지오코딩 대기인 정상 fallback

`reason`, `source_kind`, `grounding` query filter는 page items뿐 아니라 `total`, `newer_than`, cursor
fingerprint에 같은 exact 조건으로 적용한다. `grounding_status`는 T-165가 저장한 enum column을
직접 사용하며 evidence JSON에서 추정하지 않는다. scope는 T-177 `unmatched-v1`, T-182의 사유·출처
`unmatched-v2`, T-183의 검색·정렬 `unmatched-v3`, Grounding 계약을 포함한 `unmatched-v4` 순으로
올라갔고 T-184의 removed 상태 predicate에서 `unmatched-v5`가 됐다. 이전 scope cursor는 filter
의미가 달라 400으로 명시 만료한다.
...
T-183부터 검수 화면은 `listUnmatchedCandidatesPage`를 직접 사용해 oldest 기본 300개 단위로
cursor를 append한다. 검색·정렬·국내 여부·사유·출처·원문 근거·상태·그룹 filter는 URL을 단일
정본으로 쓰며,
`newer_than` probe가 새 후보와 다른 사용자 처리에 따른 queue 변화를 60초마다 확인한다. 저장·제외·
개별 삭제 뒤 현재 후보가 없고 `has_more=true`이면 다음 page를 자동으로 이어 받고,
`has_more=false`인 마지막 page까지 확인한 뒤에만 현재 표시 조건의 완료 상태를 표시한다. 일반 행
선택은 URL에 쓰지 않으며 명시적 `?candidate=`만 page 밖 상세와 filter 밖 맥락을 보존한다.

T-184부터 화면의 상태 선택은 `needs_review|removed`로 단순화한다. `removed`에서는 ignored와
soft-deleted 후보를 함께 보여주고 상세·복구만 허용하며 장소 검색·AI 의견·VWorld map 요청은 하지
않는다. 목록/상세의 `undo`는 브라우저가 해석하지 않는 descriptor이고, forward 응답 유실 복구는
`last_client_operation_id`까지 현재 요청과 정확히 일치할 때만 허용한다. 이 ID는 저장된 문자열을 그대로
노출하는 값이 아니라, operation에 기록된 결과 candidate revision과 연결 place ID/revision이 현재
목록 snapshot과 모두 같을 때만 반환되는 파생값이다. 기대 post-state인데 finalizer가 아직 끝나지 않아
ID만 null인 경우에만 클라이언트가 제한된 backoff 재조회를 수행한다.

나머지 `listRunsPage`, `listThemesPage`는 호환 wrapper로 현재 화면을 보존한다. 후속 T-190·T-192에서
각 화면이 metadata와 cursor를 직접 사용한다.
