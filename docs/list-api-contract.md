...
- 검수는 `channel_id`, `playlist_id`, `keyword`와 암묵 조건
  `needs_review AND deleted_at IS NULL`을 고정한다. T-182부터 `reason`과 `source_kind`도
  fingerprint에 포함한다.
...
`reason`과 `source_kind` query filter는 page items뿐 아니라 `total`, `newer_than`, cursor
fingerprint에 같은 조건으로 적용한다. 이 변경으로 검수 cursor scope는 `unmatched-v2`이며 T-177의
구 `unmatched-v1` cursor는 400으로 명시 만료한다. `grounding` query와 목록 field는 T-165가 raw
근거 저장 계약을 만든 뒤 추가한다. 그 전에는 임의 JSON을 신뢰한 가짜 filter를 제공하지 않으며,
예약된 `ungrounded`도 산출하지 않는다.
...
T-179부터 검수 화면은 `listUnmatchedCandidatesPage`를 직접 사용해 300개 단위로 cursor를
append한다. 저장·제외·개별 삭제 뒤 현재 visible 후보가 없고 `has_more=true`이면 다음 page를
자동으로 이어 받아 해외 숨김 조건을 통과하는 후보를 찾는다. `has_more=false`인 마지막 page까지
확인한 뒤에만 현재 표시 조건의 완료 상태를 표시한다. 서버 검색·정렬·`newer_than` 새 후보 배너와
URL filter 상태화는 T-183에서 추가한다.
