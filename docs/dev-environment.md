...
4. `http://127.0.0.1:12101/health/live`, `http://127.0.0.1:12601/health`, `http://127.0.0.1:12605` 응답을 확인합니다(dev는 내부 주소 `127.0.0.1` + 고정 12xxx 포트: API `12601`→컨테이너 `8000`, Web `12605`→컨테이너 `3000`, 외부 RustFS 서비스 `12101`/`12105`). API health(`/health`)는 버전 프리픽스 없이 노출되며, 검증 컨텍스트는 무인증입니다.
...
단순히 dev로 라이브로 띄우기만 하려면 `scripts/start-live.sh`를 사용합니다. 이 스크립트는 Docker CLI를 확인한 뒤 `scripts/stop-fixed-ports.sh`로 이 repo 소유 고정 host port `12601`/`12602`/`12605` 점유 여부를 점검합니다. **이미 사용 중이면(개발 또는 prod 인스턴스일 수 있음) 새 포트로 바꾸지 않고, 강제 종료할지 사용자에게 물어봅니다. 거부하면 기동을 중지하고 떠 있는 인스턴스를 그대로 둡니다.** 비대화형(CI 등)에서 묻지 않고 회수하려면 `FORCE_KILL_PORTS=1`을 지정합니다(없으면 안전 중지). 포트가 비어 있거나 사용자가 종료를 승인하면 `docker compose up -d --build`로 `api`/`mcp`/`scheduler`/`frontend`를 띄웁니다. RustFS S3 API `12101`과 콘솔 `12105`는 외부 고정 Docker 서비스가 소유하므로 회수 대상이 아닙니다(점유 정리 패턴은 `python-krtour-map`에서 차용). 기동 후 dev 접속은 **내부 주소 127.0.0.1 + 고정 12xxx 포트**: API `http://127.0.0.1:12601`, MCP `http://127.0.0.1:12602/mcp`, Web `http://127.0.0.1:12605`, RustFS S3 API `http://127.0.0.1:12101`, 콘솔 `http://127.0.0.1:12105`. (prod는 같은 12xxx 포트를 공식 도메인 + 프록시로 노출 — §11/ADR-28.)

```bash
./scripts/start-live.sh
```

이 저장소의 로컬(dev) 포트는 prod와 동일한 고정 12xxx 값입니다. `docker-compose.yml`은 `${API_HOST_PORT}`, `${MCP_HOST_PORT}`, `${FRONTEND_HOST_PORT}`를 repo 서비스 포트로 읽고, RustFS용 `${RUSTFS_HOST_PORT}`/`${RUSTFS_CONSOLE_HOST_PORT}`는 선택형 `embedded-rustfs` profile에서만 사용합니다. 일반 개발·검증·라이브 실행에서는 `.env.example`의 고정값 API `12601`, MCP `12602`, Web `12605`, 외부 RustFS `12101`/`12105`를 유지합니다. 포트가 이미 사용 중이면 새 포트로 바꾸지 않고, 강제 종료 여부를 사용자에게 물어본 뒤(거부 시 중지) 같은 포트로 다시 올립니다.

Compose에서 MCP 서버는 로컬 `stdio` 기본값과 달리 `streamable-http` transport로 실행하며,
dev 기준 `http://127.0.0.1:12602/mcp`로 접근합니다.
...
## 11. 프로덕션 배포 (외부 노출, 리버스 프록시 + TLS) — ADR-28

> **dev vs prod 한눈에 보기**
> - **dev(기본)**: 별도 지시가 없으면 이 repo의 실행/스크립트는 dev를 의미합니다. 여기에서 직접
>   (`bash scripts/start-live.sh` / `docker compose up` / `./ktcctl`) 띄우고, **내부 주소 `127.0.0.1`**
>   + **고정 12xxx 포트**(API `12601`, MCP `12602`, Web `12605`, RustFS `12101`/`12105`)로 접속합니다.
> - **prod**: **`kor-travel-docker-manager`**가 도커를 올리고 **공식 도메인**을 적용합니다. prod도 **같은
>   12xxx host 포트**를 쓰되, 그 포트를 공식 도메인 + TLS 리버스 프록시로 노출합니다(포트 번호는 동일,
>   접속 주소만 다름). 앱 코드 변경 없이 **환경변수 + 프록시**로만 구성합니다.

외부에 노출하는 prod는 앱/Compose가 기존 고정 host port를 그대로 띄우고, 프록시가 공개 도메인을 Host 기반으로 그 포트에 라우팅합니다. 아래 절차는 `kor-travel-docker-manager` 없이 단독 `docker compose`로 prod 구성을 재현/검증할 때의 참고 경로이며, 실제 운영 오케스트레이션은 `kor-travel-docker-manager`가 담당합니다.

> 실제 공개 도메인과 비밀은 **외부에 노출하지 않습니다.** git에 커밋하는 파일(`.env.example`, `deploy/Caddyfile`, 문서)에는 placeholder/`{$ENV}`만 두고, 실제 값은 gitignore된 `.env`(또는 `.env.production`)에만 둡니다.

### 도메인 → 포트 매핑

| 공개 도메인(예시) | → host port | 서비스 |
| --- | --- | --- |
| `<web>` | `127.0.0.1:12605` | Next.js Web + same-origin BFF |
| `<api>` | `127.0.0.1:12601` | REST API (`/api/v1`, `X-API-Key`) |
| `<mcp>` | `127.0.0.1:12602` | MCP (streamable-http `/mcp`, SSE 버퍼링 off) |
| `<s3-api>` | `127.0.0.1:12101` | RustFS S3 API / 공개 객체(미디어) |
| `<s3-console>` | `127.0.0.1:12105` | RustFS 관리 콘솔 |

RustFS 매핑: `s3-api.<...>`=S3 API/공개 객체 URL(`RUSTFS_PUBLIC_BASE_URL`), `s3.<...>`=콘솔(`RUSTFS_CONSOLE_URL`). 백엔드 boto3 연결(`RUSTFS_DOCKER_ENDPOINT`)은 같은 호스트 내부 경로(`host.docker.internal:12101`)를 유지합니다.

### 1) prod 환경변수 준비

`.env.production`(gitignore됨)을 prod 호스트에서 `.env`로 사용하거나 직접 지정합니다. 핵심 값:
...
BACKEND_ORIGIN=http://api:8000       # 컨테이너 내부
FORWARDED_ALLOW_IPS=*                # TLS 종단 프록시 뒤 X-Forwarded-* 신뢰(uvicorn이 직접 읽음)
RUSTFS_PUBLIC_BASE_URL=https://<s3-api>/kor-travel-concierge
RUSTFS_CONSOLE_URL=https://<s3-console>
MCP_WRITE_ENABLED=false              # 공개 MCP는 쓰기 끄고 프록시 인증으로 보호
```

### 2) 앱 기동 (단독 Compose 참고 경로)

두 가지 중 하나로 prod env를 적용합니다. **`--env-file`만으로는 부족합니다**: `--env-file`은 `${...}`
보간 소스만 바꾸고, 컨테이너로 들어가는 비밀 키(`GEMINI_API_KEY`, `YOUTUBE_API_KEY`, `KAKAO_*`,
`VWORLD_*` 등)는 `env_file:` 디렉티브가 결정하기 때문입니다.

```bash
# (A) 가장 단순: .env.production을 .env로 복사해 사용 (env_file과 보간이 같은 파일을 봄)
cp .env.production .env && docker compose up -d --build

# (B) 복사 없이: env_file 경로를 APP_ENV_FILE로 덮어쓰고 보간 소스도 같은 파일로 지정
APP_ENV_FILE=.env.production docker compose --env-file .env.production up -d --build
```

`RUSTFS_CONSOLE_URL`/`NEXT_PUBLIC_API_BASE_URL`/`CORS_ALLOW_ORIGINS` 등은 env-driven이라 prod 값이 컨테이너에 반영됩니다.

### 3) 리버스 프록시 (Caddy, 자동 TLS)

`deploy/Caddyfile`은 도메인을 `{$ENV}`로 읽는 자동 TLS 샘플입니다. 실제 도메인은 `.env(.production)`에서 주입합니다.

```bash
# 공개 도메인 5개의 A/AAAA 레코드(동적 DNS)가 이 호스트를 가리키고 80/443이 열려 있어야 함
caddy run --config deploy/Caddyfile --envfile .env.production
# 또는 systemd 서비스로 등록해 상시 구동
```

**MCP 보안**: MCP는 앱 자체 인증이 없어 `deploy/Caddyfile`에서 `basic_auth`로 보호합니다(기본 ON).
운영 자격증명은 `caddy hash-password`로 bcrypt 해시를 만들어 `MCP_BASIC_AUTH_HASH`(+`MCP_BASIC_AUTH_USER`)로
주입합니다(`.env.production` 참고). 미설정 시 Caddyfile의 잠금 기본 해시가 적용되어 Caddy는 정상 기동하되
아무도 인증할 수 없습니다(익명 노출 방지, fail-safe). `MCP_WRITE_ENABLED=false`도 유지합니다.
