...
VWorld 호환 `?key=` query parameter를 검증하며, Web UI에서 발급한 key는 DB에 hash와
hint만 저장하고 활성 hash 목록은 짧은 TTL로 메모리 캐시한다.
...
- 설정 모달에서 공개 API key를 생성·목록 조회·폐기하고, 생성된 key는 한 번만 평문으로
  표시한다. 로그인 시도·성공·실패·로그아웃 기록도 같은 화면에서 확인한다.
