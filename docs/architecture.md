...
T-175는 이 scope capability와 관리 UI를 제공한다. production의 `kor-travel-map`에 DB read
키를 실제 발급·교체하고 기존 consumer용 static admin entry를 제거하는 운영 전환은 T-176
완료 전까지 대기한다. 따라서 T-175 배포만으로 기존 consumer key의 권한 축소가 끝난 것은 아니다.
