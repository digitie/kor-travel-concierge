...
- **외부 공식 용어**: Next.js, React, FastAPI, SQLAlchemy, SQLite3, SpatiaLite, Playwright, MapLibre GL JS, WMTS, REST API, Gemini API, ETL.
...
4. **ETL 파이프라인**: 공식 YouTube Data API v3 검색 및 업데이트 탐색 → 자막/전사/Gemini 활용 영상 정리 및 POI 추출 → 대표 프레임 추출 → 외부 REST API 연동 주소 보정 작업 수행.
...
4. `docs/decisions.md` — ADR-1 ~ ADR-14
