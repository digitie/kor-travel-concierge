...
- **다음 착수 대상**: `docs/improvement-roadmap-2026-07.md`(Codex 리뷰 반영판)의 실행 작업 T-158~T-192가 `docs/tasks.md` 대기에 등재돼 있다 — Agent A(백엔드 상태 모델·파이프라인·정책)와 Agent B(검수 UX·공급 API·보안 표면) 두 트랙 병렬, 순서·교차 선행·파일 소유는 로드맵 §4. 최우선은 T-158(Phase -1 정책)·T-159(exclude_video hotfix)·T-174(provenance)·T-160(soft delete). 새 작업을 시작할 때는 기존 PostgreSQL/PostGIS + Linux Docker 실행 모델과 `/api/v1/features/*` 공급자 계약, 그리고 prod 공개 도메인은 ADR-28 리버스 프록시 모델을 유지한다.
