...
- **T-013 완료**: `maplibre-gl` 기반 VWorld WMTS 지도, 장소 리스트, 검수 큐, Deep Research 트리거, 작업/저장소 운영 패널, 관련 REST endpoint를 메인 화면에 연결.
- **다음 착수 대상**: T-014 Windows 및 Docker Compose 통합 검증. Windows PowerShell 기준 실행과 단일 호스트 Docker Compose 구성을 함께 확인해야 한다.
...
- Docker Compose 통합 검증, Windows Playwright E2E 검증은 각각 T-014~T-015에 남아 있다.
- `npm audit`이 Next 14 계열 보안 이슈를 보고했으나 자동 수정은 Next 16 major upgrade를 요구한다. T-020에서 별도 검토한다.
