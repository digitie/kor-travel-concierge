...
- **T-056 완료**: Windows 네이티브 실행 경로를 배제하고 실행/평가 환경을 Linux Docker 전용(Windows는 WSL2)으로 전환. PowerShell 라이브/FFmpeg 스크립트 제거, bash `scripts/verify-docker-compose.sh`·`scripts/start-live.sh` 추가, FFmpeg을 컨테이너 `/usr/bin/ffmpeg` 단일 경로로 정리, host port를 API 8000 / Frontend 3000으로 복귀, 문서를 bash/Docker/WSL2 기준으로 재작성(ADR-23).
...
- **실행 모델 결정**: 앱 런타임은 단일 호스트 Docker Compose(`docker compose up --build`)이며 backend 8000, frontend 3000, rustfs, mcp를 띄운다. Windows 사용자는 WSL2(Ubuntu) + Docker 안에서 동일한 bash 명령으로 앱을 구동한다(ADR-23). **E2E Playwright만 Windows 호스트에서 실행**한다(ADR-23 예외).
- **API 경계 결정**: REST 엔드포인트는 `/api/v1` 프리픽스 아래(`/health`·`/`는 버전 없음)이며 `X-API-Key` 인증을 받는다. `APP_ENV=local/test/e2e`는 무인증 우회, 외부 노출 배포는 `APP_ENV=production`+`API_KEYS`로 인증을 강제한다(ADR-24).
- **다음 착수 대상**: 현재 등록된 대기 작업 없음.
...
```
F:\dev\tripmate-agent\
├── frontend/             # Next.js App Router 프론트엔드 (Compose host Port: 3000)
│   ├── src/
...
│   ├── package.json
│   └── tsconfig.json
├── backend/              # FastAPI 비동기 API 백엔드 (Compose host Port: 8000)
│   ├── app/
...
└── docs/                 # 아키텍처 및 이력 관리 문서
    ...
    ├── decisions.md      # ADR 기록 (ADR-1 ~ ADR-24)
    ├── tasks.md          # 백로그 추적
    ├── journal.md        # 일지 기록
