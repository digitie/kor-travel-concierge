...
- **T-185 완료**: 검수 bulk를 서버 filter snapshot 기반 `preview`·100건 `execute`로 구현했다. 명시
  선택 최대 500건, filter 최대 10,000건(10,001번째면 전체 413), actor/action/scope/TTL 결합 확인
  token digest, operation/item/receipt ledger, 같은 request/cursor exact replay와 stale cursor 409를
  적용했다. item savepoint로 성공·충돌·실패를 분리하면서 후보별 감사·chunk 감사·receipt를 mutation과
  원자 commit한다. UI는 선택 제외·삭제·복구와 현재 filter 해외 후보 전체 제외, 정확한 preview 건수,
  chunk 진행·응답 유실 재시도·failed-only 재확인·모바일/focus 경로를 제공한다. 3개 agent의 적대적
  검토를 세 번 교차 반복하고 보강 delta를 재감사해 최종 BLOCKER/MAJOR/MINOR 0건을 확인했다.
  n150에서 migration `20260713_0027↔20260714_0028`, T-185 backend 30건·backend 전체 761건,
  frontend Vitest 229건·lint/type-check/build, Playwright 기능 44건과 기존 timing 경로 production
  재검증 3/3 통과(4 live-only skip)를 확인했다(ADR-42).
- **T-183 완료**: 검수 큐가 `q` literal 검색, oldest/newest snapshot cursor, strict 국내 여부·사유·
  출처·상태·Grounding 5상태 filter와 정확한 `total`·`newer_than`을 `unmatched-v4`로 제공한다. 화면은
  oldest FIFO 300건 append, URL 단일 정본, page 밖 딥링크, 큐 변경 배너와 권위 상세 재검증을 사용한다.
  장소 lifecycle lock·후보 `xmin` fencing·RustFS 보존, MCP 멱등 전용 index와 pending owner/lease,
  auto-match audit 단일 전이를 보강했다. Alembic은 T-170의 `0024` 뒤에 T-183의 `0023`을 직렬로
  연결해 단일 head를 유지한다. 최신 T-170 위 재배치 후 n150에서 backend 타깃 273건·변경 Python
  31개 Ruff·Alembic `head→0024→0022→head` 왕복, frontend Vitest 159건·build, Playwright
  22건(live 4건 skip)을 통과했다. backend 전체는 664건 통과했고
  선택 자막 library가 설치된 n150 환경과 `not_configured` 가정이 다른 기존 2건만 실패했다.
- **T-178 완료**: 결과 화면이 `listDestinationsPage`의 100개 cursor page를 직접 소비한다. 불러온
  항목 수와 기준 total, 더 보기·재시도·새로고침·완료/목록 변경 상태를 구분하고, 101/501번째
  장소와 page 밖 `?place=` 단건 상세에 접근할 수 있다. 표시된 항목 선택으로 범위를 명확히 하고
  다중 page 자동 polling은 T-188 SQL pushdown 전까지 수동 새로고침으로 제한했다. n150에서
  frontend 전체 검증과 Playwright 7건을 통과했다.
...
- **T-184 완료**: 검수 마지막 단건 undo와 `removed` 복구 목록을 추가했다. IGNORED·soft delete·MATCHED·USER_CORRECTED는 DB trigger revision과 opaque descriptor로 fencing해 `needs_review`에 복귀하며, forward 응답 유실은 final candidate/place snapshot에 결합한 동일 `client_operation_id`가 exact detail에서 확인될 때만 성공으로 승격한다. 후보 생성 장소만 전역 참조 0일 때 정리하고 공유·persistent·legacy 장소, 미디어 객체, 영상 제외 상태는 보존한다(ADR-40).
- **다음 착수 대상**: Agent B는 T-180~T-185를 완료했다. 다음은 T-186 review 컴포넌트 분해이며
  이후 T-187~T-192를 로드맵 순서로 진행한다. T-186에서는 기존 300ms provider debounce·abort·
  `searchNonce`의 동일 검색 재실행 계약을 보존한다. 선택 강조·폼 초기화는 즉시 처리하고 provider
  query 활성화만 `startTransition`으로 분리한다. Agent A와의 교차 선행·파일 소유는 로드맵 §4를
  따른다. 새 작업은
  PostgreSQL/PostGIS + Linux Docker 실행 모델, ADR-36 scope 계약, prod ADR-28 리버스 프록시 모델을
  유지한다.
