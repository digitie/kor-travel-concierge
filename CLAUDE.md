...
- **다음 착수 대상**: T-016 고도화 후보 검토 또는 T-020 Next.js 메이저 업그레이드 및 npm audit 대응 검토.
...
- sqlite-vec, PostgreSQL/PostGIS 전환, 멀티 워커 큐 전환은 T-016에서 실제 규모 신호가 생길 때 기준으로 검토한다.
...
└── docs/                 # 아키텍처 및 이력 관리 문서
    ...
    ├── decisions.md      # ADR 기록 (ADR-1 ~ ADR-19)
    ├── tasks.md          # 백로그 추적
    ├── journal.md        # 일지 기록
