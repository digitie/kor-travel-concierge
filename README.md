...
REST API는 `/api/v1` 프리픽스 아래에 노출되고(`/health`·`/`만 버전 없음) `X-API-Key` 인증을 받습니다. 로컬 실행(`APP_ENV=local/test/e2e`)은 인증 코드 없이 동작하고, 외부에 노출하는 배포는 `APP_ENV=production`과 `API_KEYS`를 설정합니다(ADR-24).
...
# 비-local 백엔드 호출 시에만 사용(로컬/E2E는 비워 두어 무인증)
NEXT_PUBLIC_API_KEY=
...
API는 `http://localhost:8000`, Web은 `http://localhost:3000`으로 열립니다. FFmpeg은 컨테이너 이미지(`Dockerfile.python`)가 apt로 제공하는 `/usr/bin/ffmpeg`를 사용하므로 호스트에서 별도로 준비할 필요가 없습니다.
